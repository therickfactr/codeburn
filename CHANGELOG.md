...
### Added (CLI)
- **Cline CLI sessions are now counted.** The Cline command-line agent (npm `cline`, 3.x) stores sessions as `~/.cline/data/sessions/<id>/<id>.json` + `<id>.messages.json`, a layout the Cline provider never scanned — it required `tasks/<id>/ui_messages.json` — so every CLI session was silently reported as $0.00, with no warning even under `--verbose`. Discovery now covers that layout as a third root of the existing `cline` provider (no second provider, so Cline stays one row and the CLI root stays out of the task-id dedup pass), with the CLI parsing in its own module so the shared Cline-family parser behind Roo Code, KiloCode and IBM Bob is untouched. Per-message cost is metered by the CLI, so those calls keep their reported cost rather than being re-priced from tokens, and the cline parse version is bumped so warm caches re-derive once. Model ids are also resolved to their real names now (the CLI records routed ids like `cline-pass/glm-5.2`), which also gives the extension's ids their proper labels. (#874)
