...
| **Cline / Roo Code / KiloCode** | VS Code `globalStorage` across VS Code, VS Code Insiders, and VSCodium (Cline at `saoudrizwan.claude-dev`, plus `~/.cline/data`); Cline additionally reads the CLI's `~/.cline/data/sessions/<session-id>/` layout, honoring `CLINE_SESSION_DATA_DIR`, `CLINE_DATA_DIR` and `CLINE_DIR` | Cline-family agents. CodeBurn reads `ui_messages.json` from each task directory, extracting token counts from `type: "say"` entries with `say: "api_req_started"`. Cline CLI sessions instead pair a `<id>.json` metadata file with a `<id>.messages.json` transcript, where each assistant message carries its own metered `metrics` block. |
